...
(put 'complex 'is-zero? zero-complex?)
(put 'rational 'is-zero? zero-rational?)
(put 'real 'is-zero? zero-real?)

(define (equ? x y)
  (if (and (number? x) (number? y))
    (= x y)
    (operate 'is-zero? (sub x y))))

#!

We can do the same for dropping a type:

!#

(define (proj-complex c)
  (make-real (real-part c)))

(define (proj-real r)
  (make-rational (make-fraction (* 100000 r) 100000)))

(define (proj-rational r)
  (floor r))

(put 'complex 'project proj-complex)
(put 'real 'project proj-real)
(put 'rational 'project proj-rational)

(define (project n)
  (if (not (null? (get (type n) 'drop)))
    (operate 'drop n)
    #nil))

(define (drop n)
  (if (and (not (null? project n))
...
(define (sqrt-rat r)
  (make-real (sqrt (/ (numerator r) (denominator r)))))
...
(define (sqrt-comp c)
  (make-complex (make-polar (sqrt (magnitude c)) (div (angle c) 2))))

(define (atan-rat r1 r2)
  (make-real (atan (/ (numerator r1) (denominator r1))
		   (/ (numerator r2) (denominator r2)))))

(define (atan-real r1 r2)
  (make-real (atan r1 r2)))

(define (atan-int i1 i2)
  (make-real (atan i1 i2)))

(put 'integer 'sqrt sqrt-int)
(put 'rational 'sqrt sqrt-rat)
(put 'real 'sqrt sqrt-real)
(put 'complex 'sqrt sqrt-comp)

(put 'integer 'atan atan-int)
(put 'rational 'atan atan-rat)
(put 'real 'atan atan-real)
